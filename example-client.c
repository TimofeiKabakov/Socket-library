// This is a simple example of utilizing the library to implement a simple client. 
// The client connects to a single server, whose information is given via the commandline. 
// The client sends requests to the server, then blocks until it receives a response.
...
int main(int argc, const char **argv) {
  if (argc < 3) {
    printf("Error: Not enough arguments! You must provide a server address and port to the commandline for the server you wish to connect to.\n");
